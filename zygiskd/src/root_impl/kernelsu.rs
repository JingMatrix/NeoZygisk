...
// Mirrors `struct ksu_set_feature_cmd` from KernelSU's `uapi/supercall.h`. The 4-byte
// gap the C compiler inserts before the 8-byte-aligned `value` is reproduced by
// `#[repr(C)]`, so the layout matches the kernel's.
#[repr(C)]
